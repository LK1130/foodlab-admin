...
@section('title', 'Admin | Suggest Edit')
...
@section('body')
    ...
    <div class="col-md-10 ">
        ...
        <div class="adminAddForm">
            <form action="{{ route('suggestAdmin.update', $suggest->id) }}" method="POST">
                @csrf
                @method('PUT')
                <p class="formHeader">{{ __('messageZY.editsuggest') }} </p>
                <div>
                    <div class="form-group">
                        <label for="suggest">{{ __('messageZY.suggest') }}</label>
                        <input type="text" class="form-control" id="suggest" name="suggest_type"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="note" value="{{ $suggest->note }}">
                        @error('note')
                            <li class="text-danger ">{{ $message }}</li>
                        @enderror
                    </div>
