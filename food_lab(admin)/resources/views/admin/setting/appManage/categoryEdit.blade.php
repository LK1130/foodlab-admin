    ...
    <div class="col-md-10 ">
        ...
        <div class="adminAddForm">
            <form action="{{ route('category.update', $category->id) }}" method="POST">
                @csrf
                @method('PUT')
                <p class="formHeader">{{ __('messageZY.editcategory') }} </p>
                <div>
                    <div class="form-group">
                        <label for="categoryname">{{ __('messageZY.categoryname') }}</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="note" value="{{ $category->note }}">
                        @error('note')
                            <li class="text-danger ">{{ $message }}</li>
                        @enderror
                    </div>
